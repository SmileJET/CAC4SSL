    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for _ in range(1, single_out_num):
                label_batches.append(F.interpolate(label_batches[-1].float(), scale_factor=0.5, recompute_scale_factor=False))
            ...
            loss = args.lamda * loss_seg_dice + consistency_weight * loss_consist + consistency_weight * loss_contrast
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
